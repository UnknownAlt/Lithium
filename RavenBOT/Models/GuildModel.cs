namespace RavenBOT.Models
{
    ...
    public class GuildService
    {
        ...
        public class GuildModel
        {
            ...
            public void Save()
            {
                DatabaseService.UpdateOrStoreAsync($"{ID}", this);
            }
            ...
            public class AdditionalDetails
            {
                ...
                {
                    return new AdditionalDetails { Details = new List<Detail> { new Detail { Title = title, Content = content, ShowInRegularChat = false } } };
                }
            ...
            public async Task ModActionAsync(SocketGuildUser user, SocketGuildUser moderator, SocketTextChannel channel, string reason, Moderation.ModEvent.AutoReason autoModReason, Moderation.ModEvent.EventType modAction, Moderation.ModEvent.Trigger trigger, TimeSpan? expires, AdditionalDetails additionalDetails)
            {
                ...
                var modEvent = new Moderation.ModEvent { Action = modAction, ExpiryDate = expires == null ? null : DateTime.UtcNow + expires, ModName = moderator.ToString(), ModId = moderator.Id, UserId = user.Id, UserName = user.ToString(), AutoModReason = autoModReason, ProvidedReason = reason, ReasonTrigger = trigger, ActionId = ModerationSetup.ModActions.Count };
                ...
                var embed = new EmbedBuilder { Fields = responseFields }.WithCurrentTimestamp();
                ...
                {
                    var deleteAction = Task.Delay(ModerationSetup.Settings.AutoHideDelay.Value).ContinueWith(_ => m.DeleteAsync().ConfigureAwait(false));
                }
            ...
            public async Task<ModTaskResponse> KickUserAsync(SocketGuildUser user, EmbedBuilder _embed, ISocketMessageChannel channel, RequestOptions request, string reason)
            {
                ...
                return new ModTaskResponse { embed = _embed, success = _success };
            }
            ...
            public async Task<ModTaskResponse> MuteUserAsync(SocketGuildUser user, EmbedBuilder _embed, ISocketMessageChannel channel, RequestOptions request)
            {
                ...
                if (muteRole == null)
                {
                    ...
                    if (muteRole == null)
                    {
                        ...
                        await channel.SendMessageAsync("", false, new EmbedBuilder { Description = $"Muted Role was not configured, I have auto-generated one for you. {muteRole.Mention} has been auto-set as the server's mute role" }.Build());
                        ModerationSetup.Settings.MutedRoleId = muteRole.Id;
                    }
                    else
                    {
                        await channel.SendMessageAsync("", false, new EmbedBuilder { Description = $"Muted Role was not configured, but one was found. {muteRole.Mention} has been auto-set as the server's mute role" }.Build());
                        ModerationSetup.Settings.MutedRoleId = muteRole.Id;
                    }
                ...
                return new ModTaskResponse { embed = _embed, success = successResult };
            }
