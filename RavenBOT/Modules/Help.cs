namespace RavenBOT.Modules
{
    ...
    public class Help : Base
    {
        ...
        /// The provider.
        /// </param>
        private Help(CommandService commandService, PrefixService prefix, IServiceProvider provider)
        {
        ...
        public Task ModuleCommandHelpAsync(bool checkPreconditions, string checkForMatch)
        {
            ...
            var prefix = prefixService.GetPrefix(Context.Guild?.Id ?? 0);
            if (module != null)
            {
                ...
                var info = passingCommands.Select(x => $"`{prefix}{x.Aliases.FirstOrDefault()} {string.Join(" ", x.Parameters.Select(ParameterInformation))}` {(x.Summary != null ? $"\nS: {x.Summary}" : "")} {(x.Remarks != null ? $"\nR: {x.Remarks}" : "")}").ToList();
            ...
            var command = service.Search(Context, Context.Message.Content.Substring(Command.Aliases.First().Length + prefix.Length + 1)).Commands?.FirstOrDefault().Command;
            if (command != null)
            {
                    ...
                    fields.Add(new EmbedFieldBuilder
                                   {
                                       ...
                                       Value = "**Usage:**\n" +
                                               $"{prefix}{command.Aliases.FirstOrDefault()} {string.Join(" ", command.Parameters.Select(ParameterInformation))}\n" +
                                               "**Aliases:**\n" +
                                               $"{string.Join("\n", command.Aliases)}\n" +
                                               "**Module:**\n" +
                                               $"{command.Module.Name}\n" +
        ...
        public Task PagedHelpAsync(bool checkPreconditions)
        {
                                         ...
                                         new EmbedFieldBuilder
                                             {
                                                 ...
                                                 Value = "Go to the respective page number of each module to view the commands in more detail. " +
                                                         ...
                                                         $"type `{prefix}help <ModuleName>` or `{prefix}help <CommandName>`"
                                             }
                                     };
            ...
            foreach (var moduleSet in moduleSets)
            {
                ...
                foreach (var module in moduleSet)
                {
                    ...
                    try
                    {
                        ...
                        var summary = passingCommands.Select(x => $"{prefix}{x.Aliases.FirstOrDefault()} {string.Join(" ", x.Parameters.Select(ParameterInformation))}").ToList();
            ...
            return PagedReplyAsync(new PaginatedMessage { Pages = pages, Title = $"{Context.Client.CurrentUser.Username} Help || Prefix: {prefix}", Color = Color.DarkRed }, new ReactionList { Backward = true, Forward = true, Jump = true, Trash = true });
        }
